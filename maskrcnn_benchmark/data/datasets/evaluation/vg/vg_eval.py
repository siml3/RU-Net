...
from maskrcnn_benchmark.data.datasets.evaluation.vg.sgg_eval import SGRecall, SGNoGraphConstraintRecall, SGZeroShotRecall, SGNGZeroShotRecall, SGPairAccuracy, SGMeanRecall, SGNGMeanRecall, SGAccumulateRecall, SGMeanMicroRecall
...
def do_vg_evaluation(
    ...
    if "relations" in iou_types:
        ...
        if cfg.MODEL.ROI_RELATION_HEAD.L21_LOSS is not None and cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX:
            eval_recall.l21_thr = cfg.MODEL.ROI_RELATION_HEAD.L21_LOSS
        ...
        eval_mean_micro_recall = SGMeanMicroRecall(result_dict, num_rel_category, dataset.ind_to_predicates, print_detail=True)
        eval_mean_micro_recall.register_container(mode)
        evaluator['eval_mean_micro_recall'] = eval_mean_micro_recall
        ...
        eval_mean_micro_recall.calculate_mean_recall(mode)
        
        # print result
        result_str += eval_recall.generate_print_string(mode)
        result_str += eval_zeroshot_recall.generate_print_string(mode)
        result_str += eval_mean_recall.generate_print_string(mode)
        result_str += eval_mean_micro_recall.generate_print_string(mode)   
...
def evaluate_relation_of_one_image(groundtruth, prediction, global_container, evaluator):
    ...
    evaluator['eval_mean_micro_recall'].collect_mean_recall_items(global_container,
