...
#     pred_boxes = BoxList(pred_boxes, (0, 0), 'xyxy')
#     ious = boxlist_iou(pred_boxes,pred_boxes)
#     sub_ious = ious[pred_rel_inds[:,0]][:,pred_rel_inds[:,0]].cpu().numpy()
#     obj_ious = ious[pred_rel_inds[:,1]][:,pred_rel_inds[:,1]].cpu().numpy()
...
#     l21 = np.sqrt((np.power(rel_scores.cpu().numpy()[:,None,:],2.0)+np.power(rel_scores.cpu().numpy()[None,:,:],2.0))).sum(axis=-1)
...
#     rel_scores_cp = rel_scores.cpu().numpy()
#     rel_scores_rt = rel_scores_cp.copy()
#     rel_scores_cp[:,0]=0.
...
#         rel_scores_rt[is_overlap[box_ind,:,cls_ind],cls_ind]=0.0
#         rel_scores_cp[box_ind] = -1.

#     pred_rels = torch.from_numpy(pred_rels).to(pred_classes)
#     rel_scores = torch.from_numpy(rel_scores_rt).to(rel_scores)
#     return  pred_rels, rel_scores[torch.arange(pred_rels.shape[0]).to(pred_rels),pred_rels], rel_scores
