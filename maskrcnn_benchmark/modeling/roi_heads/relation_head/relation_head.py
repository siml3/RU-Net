...
from .L21_loss import L2ClearBatchLoss
...
class ROIRelationHead(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        if cfg.MODEL.ROI_RELATION_HEAD.L21_LOSS:
            alter_norm = cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR in ("TransformerPredictor")
            self.l2_loss = L2ClearBatchLoss(alter_norm)
        else:
            self.l2_loss = None
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        if self.training:
            ...
            with torch.no_grad():
                ...
                if self.cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR in ("RUNetPredictor","RUCausalAnalysisPredictor"):
                    full_pair_idxs = self.samp_processor.prepare_test_pairs(features[0].device, proposals)
        else:
            rel_labels, rel_binarys = None, None
            rel_pair_idxs = self.samp_processor.prepare_test_pairs(features[0].device, proposals)
            full_pair_idxs = rel_pair_idxs
        ...
        if self.use_union_box:
            if self.cfg.MODEL.ROI_RELATION_HEAD.PREDICTOR in ("RUNetPredictor","RUCausalAnalysisPredictor"):
                union_features = self.union_feature_extractor(features, proposals, full_pair_idxs)
            else:
        ...
        elif self.l2_loss is not None:
            l2_loss = self.l2_loss(proposals, relation_logits, rel_pair_idxs, rel_labels)
        ...
        if self.l2_loss is not None:
            output_losses["l2_loss"]=l2_loss*0.1
